...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		else throw new DbException("got unexpected pid (neither Internal nor Lead) in findLeadPage in BTreeFile.java\n");

		Iterator<BTreeEntry> entryIterator = ((BTreeInternalPage)tmpPage).iterator();
		BTreeEntry tmpEntry = null;
		IndexPredicate targetFieldPredicate = new IndexPredicate(Op.LESS_THAN_OR_EQ, f);
		if (f == null)
		{
		...
		{
			tmpEntry = entryIterator.next();
			IndexPredicate tmpFieldPredicate = new IndexPredicate(Op.LESS_THAN_OR_EQ, tmpEntry.getKey());
			if (targetFieldPredicate.equals(tmpFieldPredicate))
			{
				return findLeafPage(tid, dirtypages, tmpEntry.getLeftChild(), perm, f);
			}
		}
		if (tmpEntry == null)
		{
			throw new DbException(String.format("Got 0 entries in page with page ID: %s in findLeafPage() \n", tmpPage.getId().toString()));
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		if (page.getRightSiblingId() != null)
		{
			...
			BTreeLeafPage originalRightPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			dirtypages.put(originalRightPage.getId(), originalRightPage);
			originalRightPage.setLeftSiblingId(newRightPage.getId());
		}
		page.setRightSiblingId(newRightPage.getId());

		int numTuplesTotal = page.getNumTuples();
		int numTuplesInRightPage = numTuplesTotal / 2;

		Iterator<Tuple> moveIterator = page.reverseIterator();
		Tuple tmpTuple = null;
		for (int i = 0; i < numTuplesInRightPage; i++)
		{
			tmpTuple = moveIterator.next();
			page.deleteTuple(tmpTuple);
			newRightPage.insertTuple(tmpTuple);
		}

		Tuple middleTuple = moveIterator.next();
		...
		parentPage.insertEntry(copyUpKeyEntry);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numEntriesTotal = page.getNumEntries();
		int numEntriesInRightPage = (numEntriesTotal - 1) / 2;

		Iterator<BTreeEntry> moveIterator = page.reverseIterator();
		BTreeEntry tmpEntry = null;
		for (int i = 0; i < numEntriesInRightPage; i++)
		{
			tmpEntry = moveIterator.next();
			page.deleteKeyAndRightChild(tmpEntry);
			newInternalPage.insertEntry(tmpEntry);
		}
		BTreeEntry pushUpKeyEntry;
		tmpEntry = moveIterator.next();
		pushUpKeyEntry = new BTreeEntry(tmpEntry.getKey(), page.getId(), newInternalPage.getId());
		page.deleteKeyAndRightChild(tmpEntry);
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), pushUpKeyEntry.getKey());
		dirtypages.put(parentPage.getId(), parentPage);
		parentPage.insertEntry(pushUpKeyEntry);
		page.setParentId(parentPage.getId());
		newInternalPage.setParentId(parentPage.getId());
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, newInternalPage);

		IndexPredicate middlePredicate = new IndexPredicate(Op.LESS_THAN, pushUpKeyEntry.getKey());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int moveNum = (pageTupleNum + siblingTupleNum) / 2 - pageTupleNum;

		Tuple tmpTuple = null;
		if (isRightSibling)
		{
			Iterator<Tuple> moveIterator = sibling.iterator();
		...
		{
			Iterator<Tuple> moveIterator = sibling.reverseIterator();
			for (int i = 0; i < moveNum; i++)
			{
				tmpTuple = moveIterator.next();
				sibling.deleteTuple(tmpTuple);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		int pageEntryNum = page.getNumEntries();
		int siblingEntryNum = leftSibling.getNumEntries();
		int moveNum = (pageEntryNum + siblingEntryNum) / 2 - pageEntryNum - 1; // Since we will gen a new entry

		dirtypages.put(page.getId(), page);
		dirtypages.put(leftSibling.getId(), leftSibling);
		dirtypages.put(parent.getId(), parent);

		BTreePageId rightestPageOfLeft = leftSibling.reverseIterator().next().getRightChild();
		BTreePageId leftestPageOfRight = page.iterator().next().getLeftChild();
		BTreeEntry generatedEntry = new BTreeEntry(parentEntry.getKey(), rightestPageOfLeft, leftestPageOfRight);
		page.insertEntry(generatedEntry);

		BTreeEntry tmpEntry = null;
		Iterator<BTreeEntry> moveIterator = leftSibling.reverseIterator();
		for (int i = 0; i < moveNum; i++)
		{
			tmpEntry = moveIterator.next();
			BTreeEntry tmpNewEntry = new BTreeEntry(tmpEntry.getKey(), tmpEntry.getLeftChild(), tmpEntry.getRightChild());
			leftSibling.deleteKeyAndRightChild(tmpEntry);
			page.insertEntry(tmpNewEntry);
		}

		BTreeEntry pushUpEntry = moveIterator.next();
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreePageId rightestPageOfLeft = rightSibling.iterator().next().getLeftChild();
		BTreePageId leftestPageOfRight = page.reverseIterator().next().getRightChild();
		BTreeEntry generatedEntry = new BTreeEntry(parentEntry.getKey(), leftestPageOfRight, rightestPageOfLeft);
		page.insertEntry(generatedEntry);

		BTreeEntry tmpEntry = null;
		Iterator<BTreeEntry> moveIterator = rightSibling.iterator();
		for (int i = 0; i < moveNum; i++)
		{
			tmpEntry = moveIterator.next();
			BTreeEntry tmpNewEntry = new BTreeEntry(tmpEntry.getKey(), tmpEntry.getLeftChild(), tmpEntry.getRightChild());
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		{
			BTreeLeafPage rightOfRightPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			dirtypages.put(rightOfRightPage.getId(), rightOfRightPage);
			rightOfRightPage.setLeftSiblingId(leftPage.getId());
		}

		Iterator<Tuple> moveIterator = rightPage.iterator();
		Tuple tmpTuple = null;
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreePageId rightestPageOfLeft = rightPage.iterator().next().getLeftChild();
		BTreePageId leftestPageOfRight = leftPage.reverseIterator().next().getRightChild();
		BTreeEntry generatedEntry = new BTreeEntry(parentEntry.getKey(), leftestPageOfRight, rightestPageOfLeft);
		leftPage.insertEntry(generatedEntry);

		BTreeEntry tmpEntry = null;
		Iterator<BTreeEntry> moveIterator = rightPage.iterator();
		while (moveIterator.hasNext())
		{
			tmpEntry = moveIterator.next();
			BTreeEntry tmpNewEntry = new BTreeEntry(tmpEntry.getKey(), tmpEntry.getLeftChild(), tmpEntry.getRightChild());
			rightPage.deleteKeyAndLeftChild(tmpEntry);
			leftPage.insertEntry(tmpNewEntry);
		}
